...
public class MenuExample extends Application{
	...
	public void start(Stage primaryStage) throws Exception {
		...
		menuBar.prefWidthProperty().bind(primaryStage.widthProperty()); // binds menu bar width with primary stage width
		root.setTop(menuBar);
		...
		fileMenu.getItems().addAll(newMenuItem, new SeparatorMenuItem(), saveMenuItem, exitMenuItem);
		...
		RadioMenuItem stopAlarmItem = new RadioMenuItem("Stop Alarm");
		...
		//contingency menu
		Menu contingencyMenu = new Menu("Contingency Plans");
		contingencyMenu.getItems().addAll(new CheckMenuItem("Check 1"), new CheckMenuItem("Check 2"), new CheckMenuItem("Check 3"));
		
		alarmMenu.getItems().addAll(soundAlarmItem, stopAlarmItem, contingencyMenu);
		
		menuBar.getMenus().addAll(fileMenu, cameraMenu, alarmMenu);
		
		//root.setTop(new MenuBar()); put everything above into a separate class and boom ez pz.
